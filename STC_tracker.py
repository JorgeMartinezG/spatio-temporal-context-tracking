...
if __name__ == '__main__':
    ...
    frames_list = os.listdir(os.path.join(dataset_folder, video_name))
    frames_list.sort()
    ...
    # Initial target size.
    target_sz = [initstate[3], initstate[2]] 
    
    # Parameters according to the paper.
    padding = 1                                 # Extra area.
    rho = 0.075                                 # Learning parameter rho.
    sz = math.floor(target_sz * (1. + padding))  # Context region size.
    ...
    #rs, cs.
    #dist = rs^2 + cs^2
    #conf = math.exp(-0.5 / (alapha) * math.sqrt(dist))
    ...
    for frame in frames_list:
        img = cv2.imread(os.path.join(dataset_folder, video_name, frame))
